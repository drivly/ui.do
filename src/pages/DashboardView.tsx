...
export function DashboardView({ domain, nouns, isAdmin, onNavigate }: Props) {
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">{domain.title || domain.slug}</h1>
        <p className="text-sm text-gray-500">{nouns.length} entities</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {nouns.map(n => (
          <button key={n.id}
            ...
            <div className="text-sm font-medium text-gray-900">{n.plural || n.name}</div>
            <div className="text-xs text-gray-400 mt-1">View all</div>
          </button>
        ))}

        <button
          onClick={() => onNavigate({ type: 'uod' })}
          ...
          <div className="text-xs text-gray-400 mt-1">View Universe of Discourse</div>
        </button>
      </div>
